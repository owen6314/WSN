...
class Node 
{
        ...
        if (newEnd > dataStart + data.length) 
        {
            ...
            if (data.length < MAX_SIZE)
            {
                ...
                if (newLength >= MAX_SIZE)
                    newLength = MAX_SIZE;
                SensorData[] newData = new SensorData[newLength];
    ...
    /* Data received containing NREADINGS samples from messageId * NREADINGS 
       onwards */
    void update(int messageId, SensorData readings)
    {
        System.out.println("Node is updated with messageId " + messageId);
        int start = messageId * Constants.NREADINGS;
        setEnd(start, start + Constants.NREADINGS);
        System.out.println("Start is " + start);
        System.out.println("dataStart is " + dataStart);
        System.out.println("dataEnd is " + dataEnd);
        data[start - dataStart] = readings;
    }

    /* Return value of sample x, or -1 for missing data */
    SensorData getData(int x) 
    {
